...
type Orchestrator interface {
	StartScrape(username, platform string, saveText bool)
	CancelScrape(username string) bool
	CancelAll()
	Subscribe() (chan SSEEvent, func())
	GetAllProgress(accounts []config.Account) []TaskProgress
	SyncTargets(accounts []config.Account)
	Shutdown()
...
func InitOrchestrator(cfg config.Store, mediaIndex gallery.MediaIndex) {
	scrapers := scraper.BuildAll(cfg)
...
func loadPersistedSyncInfo(cfg config.Store, username string) (string, time.Time) {
	c := cfg.Get()
...
func SavePersistedSyncInfo(cfg config.Store, username, status string, updatedAt time.Time) {
...
// SavePersistedSyncMode records the Instagram backend used for the last sync
func SavePersistedSyncMode(cfg config.Store, username, mode string) {
	cfg.Update(func(c *config.Config) {
		for i, acc := range c.Accounts {
			if strings.EqualFold(acc.Username, username) {
				c.Accounts[i].LastSyncMode = mode
				return
			}
		}
	})
...
func (o *orchestrator) SyncTargets(accounts []config.Account) {
	...
	for _, acc := range accounts {
		...
		if _, exists := o.progress[acc.Username]; !exists {
			status, updatedAt := loadPersistedSyncInfo(o.cfg, acc.Username)
			// Stale "running" status from a previous instance — reset to idle
			if status == "running" {
				status = "idle"
				SavePersistedSyncInfo(o.cfg, acc.Username, "idle", updatedAt)
			}
	...
	for k := range o.progress {
		if !current[k] {
			SavePersistedSyncInfo(o.cfg, k, o.progress[k].Status, o.progress[k].UpdatedAt)
			delete(o.progress, k)
		}
	}
}
...
func (h *WebLogHandler) Handle(ctx context.Context, r slog.Record) error {
	var username string
	...
	if username != "" {
		h.orchestrator.AppendTaskLog(username, r.Time, r.Level.String(), r.Message)
	}
...
func (o *orchestrator) AppendTaskLog(username string, t time.Time, level, msg string) {
	...
	if status == "running" {
		...
		if containsIgnoreCase(msg, "navigating") || containsIgnoreCase(msg, "testing twitter mirror") || containsIgnoreCase(msg, "instagram session is valid") {
			progress = 20
		} else if containsIgnoreCase(msg, "timeline page parsed") || containsIgnoreCase(msg, "instagram post count") || containsIgnoreCase(msg, "instagram feed page parsed") || containsIgnoreCase(msg, "resolved instagram user id") {
			progress = 50
		} else if containsIgnoreCase(msg, "concurrent download workers") {
			progress = 70
		} else if containsIgnoreCase(msg, "file downloaded") || containsIgnoreCase(msg, "instagram file downloaded") {
			if progress < 95 {
				progress += 2
			}
		}
...
func (o *orchestrator) StartScrape(username string, platform string, saveText bool) {
	...
	if platform == "instagram" {
		...
		if !forceFullInstagramSync {
			c := o.cfg.Get()
			currentMode := c.InstagramMode
			if currentMode == "" {
				currentMode = "picuki"
			}
			var prevMode string
			...
			if prevMode != currentMode {
				forceFullScrapeReason = fmt.Sprintf("prev_mode=%s new_mode=%s", prevMode, currentMode)
				lastSync = time.Time{}
			}
		}
	}
	...
	if forceFullScrapeReason != "" {
		slog.Info("Instagram backend mode changed since last sync, forcing full re-scrape", "user", username, "reason", forceFullScrapeReason)
	}
	SavePersistedSyncInfo(o.cfg, persistUser, persistStatus, persistUpdated)
	go func() {
		...
		var err error
		s, ok := o.scrapers[platform]
		if !ok {
			err = fmt.Errorf("unknown platform: %s", platform)
		} else {
			err = s.Scrape(timeoutCtx, username, saveText, lastSync)
		}
		o.mu.Lock()
		...
		if err != nil {
			...
			p.AuthError = scraper.IsAuthExpiredError(err)
		} else {
			p.Status = "completed"
			p.Progress = 100
			p.AuthError = false
		...
		SavePersistedSyncInfo(o.cfg, p.Username, p.Status, p.UpdatedAt)
		if err == nil && platform == "instagram" {
			mode := o.cfg.Get().InstagramMode
			if mode == "" {
				mode = "picuki"
			}
			SavePersistedSyncMode(o.cfg, p.Username, mode)
		}
		o.mu.Unlock()
