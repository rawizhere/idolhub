...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("keydown", (e) => {
    ...
    if (e.key === "/") {
      const target = document.getElementById("gallery-search-input");
      if (target) {
        e.preventDefault();
        target.focus();
      }
...
let targetPlatformFilter = "all";

function filterTargets() {
  const input = document.getElementById("target-search-input");
  targetSearchQuery = (input ? input.value : "").toLowerCase().trim();
  renderDashboardSidebar();
}
...
function renderDashboardSidebar() {
  ...
  if (!container) return;
  const filtered = cachedProgress.filter(function (t) {
    if (targetPlatformFilter !== "all" && t.platform !== targetPlatformFilter) return false;
    if (targetSearchQuery && t.username.toLowerCase().indexOf(targetSearchQuery) === -1) return false;
    return true;
  });
...
function updateTerminal() {
  ...
  if (terminalLevel !== "all") {
    filtered = lines.filter(l => l && l.level && l.level.toLowerCase() === terminalLevel);
  }

  if (filtered.length > 0) {
    console.log("Terminal logs:", filtered.slice(-5).map(l => ({ level: l.level, message: l.message })));
  }
  ...
  if (lines.length > prevCount) {
    for (let i = prevCount; i < lines.length; i++) {
      if (lines[i] && lines[i].level && lines[i].level.toLowerCase() === "error") { sawNewError = true; break; }
    }
  }
...
function toggleSettings() {
  const p = document.getElementById("settings-modal");
  if (p) p.style.display = (p.style.display === "none" || !p.style.display) ? "flex" : "none";
}
window.toggleSettings = toggleSettings;

function toggleSettingsModal() { toggleSettings(); }
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape") {
    ...
    if (s && s.style.display === "flex") { s.style.display = "none"; return; }
...
async function renderGalleryGrid() {
  ...
  const tags = selectedHashtags.size > 0 ? Array.from(selectedHashtags).join(",") : "all";
  const url = `/gallery/${encodeURIComponent(activeGalleryPlatform)}/${encodeURIComponent(activeGalleryUser)}?filter=${encodeURIComponent(currentFilter)}&q=${encodeURIComponent(gridSearchQuery)}&sort=${postsSortAsc ? "asc" : "desc"}&year=${encodeURIComponent(year)}&month=${encodeURIComponent(month)}&tags=${encodeURIComponent(tags)}`;
  try {
...
async function renderGalleryPosts() {
  ...
  const month = selectedMonths.size > 0 ? Array.from(selectedMonths).join(",") : "all";
  const tags = selectedHashtags.size > 0 ? Array.from(selectedHashtags).join(",") : "all";
  const url = `/gallery/${encodeURIComponent(activeGalleryPlatform)}/${encodeURIComponent(activeGalleryUser)}/posts/page/1?sort=${postsSortAsc ? "asc" : "desc"}&q=${encodeURIComponent(postsSearchQuery)}&year=${encodeURIComponent(year)}&month=${encodeURIComponent(month)}&tags=${encodeURIComponent(tags)}`;
  try {
    const res = await fetch(url);
    const html = await res.text();
    container.innerHTML = html;
...
function switchGalleryView(view) {
  currentView = view;
  setActiveBtn('.gallery-tab', 'id', `tab-${view}`);
...
function applyGalleryFilter(filter) {
  ...
  setActiveBtn('.gallery-filter-btn', 'data-filter', filter);
  if (currentView === "grid") {
    renderGalleryGrid();
  }
}
...
function setActiveBtn(selector, attr, value) {
...
// Unified search for both Posts and Media views
const applyGallerySearch = debounce(() => {
...
function renderOverviewDashboard() {
  ...
  if (lastSyncTime) {
    ...
    if (cd === null) {
      ...
      countdownText = `${cd.hours > 0 ? cd.hours + "h " : ""}${cd.minutes}m left`;
    }
  }
...
window.addEventListener("click", (e) => {
  ...
  // Target actions menu (mobile)
  const ad = document.getElementById("actions-dropdown");
  if (ad && !ad.contains(e.target) && e.target.closest("#actions-dropdown") === null) {
    const trigger = e.target.closest('[onclick*="toggleActionsMenu"]');
    if (!trigger) ad.classList.add("hidden");
  }
});
...
function updateFilterButtonLabel(type) {
  const prefix = type === "year" ? "year" : "month";
  const sel = type === "year" ? selectedYears : selectedMonths;
  const labelEl = document.getElementById(`${prefix}-filter-label`);
